...
if( experiment is None):
    experiment_id = create_mlflow_experiment(experiment_name=experiment_name, artifact_location='app_predict_artifacts', tags={"env": "dev", "version": "1.0.0"})
...
with mlflow.start_run(run_name=run_name, experiment_id=experiment.experiment_id) as run:
    ...
    df = df.drop(columns=["App", "Unnamed: 0", ])
    ss = StandardScaler()
    ...
    mlflow.sklearn.log_model(sk_model=xgb, artifact_path="decision_tree_classifier", signature=model_signature)
